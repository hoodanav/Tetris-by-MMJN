...
public class TetrisPiece implements Serializable {
    ...
    */
    ...
    public TetrisPiece(TetrisPoint[] points) {
        ...
        int high_x = this.body[0].x;
        int low_x = this.body[0].x;
        int high_y = this.body[0].y;
        int low_y = this.body[0].y;
        for (TetrisPoint point: this.body) {
            if (point.x >= high_x) high_x = point.x;
            if (low_x >= point.x) low_x = point.x;
            if (point.y >= high_y) high_y = point.y;
            if (low_y >= point.y) low_y = point.y;
        }
        this.height = high_y + 1 - low_y;
        this.width = high_x + 1 - low_x;
        this.lowestYVals = new int[this.width];

        for (int i = low_x; i <= high_x; i++) {
            int lowest = high_y;
            for (TetrisPoint point: this.body) if (point.x == i && point.y <= lowest) lowest = point.y;
            this.lowestYVals[i-low_x] = lowest;
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof TetrisPiece piece)) return false;
        List<TetrisPoint> lst = new ArrayList<>(Arrays.asList(this.body));
        for (TetrisPoint point: piece.body) {
            if (!lst.contains(point)) return false;
        }
        return true;
    ...
    public TetrisPiece computeNextRotation() {
        ...
        int shift = highest_y(this.body);
        int count = 0;

        for (TetrisPoint point: this.body) {
            TetrisPoint new_point = new TetrisPoint(-point.y, point.x);
            new_point.x += shift;
            new_piece[count] = new_point;
            count++;
        }
        return new TetrisPiece(new_piece);
    }

    /**
     * Find the point with the highest y value.
     *
     * @param body a TetrisPoint array containing all the points in the piece.
     * @return an integer representing the highest y value.
     */
    private int highest_y(TetrisPoint[] body) {
        TetrisPoint closest = body[0];
        for (TetrisPoint point: body) {
            if (closest.y < point.y) closest = point;
        }
        return closest.y;
    }
